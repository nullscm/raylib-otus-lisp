...
(if (not (file-exists? "raylib_api.xml"))
  (syscall
    1017
    (c-string
      "wget https://raw.githubusercontent.com/raysan5/raylib/master/parser/output/raylib_api.xml")
    #f
    #f))
(define xml
...
(define (generate-colors port)
  (for-each
    (lambda (x)
      (display "(define " port)
      (display (car x) port)
      (display " (rgba->hex" port)
      (for-each
        (lambda (y) (display " " port) (display y port))
        (cdr x))
      (display "))" port))
    %colors))
